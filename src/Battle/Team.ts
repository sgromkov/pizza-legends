...
export class Team {
  ...
  constructor(teamType: TeamType, name: string) {
  ...
  createElement() {
    ...
    this.element.setAttribute('data-team', this.teamType);
  ...
  update() {
    this.combatants.forEach((combatant) => {
      ...
      icon.setAttribute('data-dead', String(combatant.hp <= 0));
      icon.setAttribute('data-active', String(combatant.isActive));
    });
  }
